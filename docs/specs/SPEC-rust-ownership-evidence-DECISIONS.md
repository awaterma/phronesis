...
## D12. **AMENDS** — the §6.2 "derived" relations are emitted as BASE edges

**Problem.** The spec calls `filter_before_clone`, `clone_before_await`,
`read_before_mutation`, and `lock_scope_ends_before_await` "derived". The
codebase has a literal `Edge::derived` with `d: true`, and using it here is
**silently destructive**:

- `store::compact` filters `fresh.into_iter().filter(|e| !e.d)` — a derived
  edge passed in as fresh is **discarded with no error**.
- `sync::persist` regenerates derived edges only from `derive::derive_all(&base)`,
  which by contract works from the edge set alone, with no syntax tree and no
  I/O.
- Derived edges get `src: ""`, and `Edge::to_fact` then stamps
  `source: Some("graph:structural")` instead of `graph:<file>`.

All four ownership derivations need the syntax tree (a shared expression chain,
a root place, a narrowest enclosing block). They are **not** recomputable from
edges, so they cannot live in `derive_all`.

**Decision.** Every ownership relation — base and derived alike — is emitted as
...
1. `src` is the **only** compaction key, so file replacement removes stale
   ownership sites for free — which is exactly what §13.1 and §15 demand.
2. Provenance stays `graph:<file>`, which Addendum A.4 requires.
3. Nothing is added to `derive.rs`. An implementer who "helpfully" adds an
   ownership case to `derive_all` has introduced a bug.
