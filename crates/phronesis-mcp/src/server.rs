...
impl EpistemeMcp {
    ...
    #[tool(
        description = "Query the structural code graph at `.phronesis/graph.jsonl` — a map of the codebase built by the PostToolUse sensor. Answers questions about code structure without reading source files. Relations: `defines_fn` [file, function], `tested_by` [function, test] (a test that calls it *directly*), `untested` [function], `calls_api` [function, api] (panicking APIs: unwrap/expect/panic/todo/unimplemented — Rust only, there is no defensible Python equivalent), `imports` [module, module], `in_cycle` [module, cycle_id], `file_type` [file, production|test|example|build], `declares_module` [file, module]. Use `\"*\"` for an unconstrained position. Worked examples: which tests cover a function -> relation `tested_by`, args `[\"my_fn\", \"*\"]` (use this to pick which tests to run after a change); what depends on a module -> relation `imports`, args `[\"*\", \"rust:phronesis::wme\"]`; untested functions in a file -> relation `defines_fn`, args `[\"src/x.rs\", \"*\"]` then cross-check `untested`. Omit `relation` to list the vocabulary. Rust and Python are both extracted. Entities are named `<lang>:<package>[#<target>]::<module path>` — e.g. `rust:phronesis::wme`, `python:my-dist::pkg::mod` — so query with that form, not a bare module path. `tested_by` is a direct-call heuristic, so transitively-covered code can still appear untested. Requires `phr-mcp graph rebuild` if the graph has never been built."
    )]
