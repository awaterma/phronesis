...
"#;
const BRANCH: &str = "branch:src/lib.rs::safe_divide:cd6054b02dde";
...
fn c9_torn_write_between_renames_is_corrupt_not_fresh() {
    ...
    write_store(
        root.path(),
        &[hit("old_test", "fn:src/lib.rs::other", "region", &old)],
        &index(&old),
    )
    .unwrap();
    ...
    let g = gaps(&facts);
    assert!(
        g.contains(&"fn:src/lib.rs::safe_divide".to_string()) && g.contains(&BRANCH.to_string()),
        "{g:?}"
    );
}
    ...
    let bad: Vec<(&str, HitRecord)> = vec![
        (
            ...
            HitRecord {
                ...
                ..hit("t", "fn:src/lib.rs::f", "region", &rev)
            },
        ),
        (
            "bogus hit_kind",
            hit("t", "fn:src/lib.rs::f", "bogus", &rev),
        ),
        (
            "short revision",
            HitRecord {
        ...
        (
            "branch kind, fn id",
            hit("t", "fn:src/lib.rs::f", "branch", &rev),
        ),
        (
            "empty tool",
            HitRecord {
...
fn c10_import_rejects_empty_and_mixed_tools() {
    ...
    let other = HitRecord {
        ...
        ..hit("u", "fn:src/lib.rs::g", "region", &rev)
    };
    let err = import_export(
        root.path(),
        &write_export(
            exp.path(),
            &[hit("t", "fn:src/lib.rs::f", "region", &rev), other],
        ),
        1,
    )
    .unwrap_err()
...
fn c10_import_dedupes_identical_records() {
    ...
    let root = tempfile::tempdir().unwrap();
    let exp = tempfile::tempdir().unwrap();
    let r = hit("t", "fn:src/lib.rs::f", "region", &rev);
...
fn c10_import_canonicalizes_revision_to_lowercase() {
    ...
    let summary = import_export(
        ...
        &write_export(
            exp.path(),
            &[
                hit("t", "fn:src/lib.rs::f", "region", &upper),
                hit("u", "fn:src/lib.rs::f", "region", &lower),
            ],
        ),
        1,
    )
...
fn d3_select_labels_stale_evidence() {
    ...
    assert!(
        fresh_json["coverage_note"].is_null(),
        "no coverage_note for a fresh store: {fresh_json}"
    );
